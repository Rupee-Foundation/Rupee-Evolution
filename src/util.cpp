...
#else
    const char* pszModule = "rupeeevolution";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\RupeeEvolution
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\RupeeEvolution
    // Mac: ~/Library/Application Support/RupeeEvolution
    // Unix: ~/.rupeeevolution
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RupeeEvolution";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "rupeeevolution.conf"));
...
//
// Write rupeeevolution.conf by CircuitBreaker88
//
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
  ...
  if (!streamConfig.good()){
       // Create empty rupeeevolution.conf if it does not exist
       FILE* configFile = fopen(GetConfigFile().string().c_str(), "a");
       if (configFile != NULL) {
           WriteConfigFile(configFile);
          // fclose(configFile);
           printf("WriteConfigFile() rupeeevolution.conf Setup Successfully!");
           ReadConfigFile(mapSettingsRet, mapMultiSettingsRet);
       } else {
           printf("WriteConfigFile() rupeeevolution.conf file could not be created");
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "rupeeevolutiond.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong RupeeEvolution will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("RupeeEvolution"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
