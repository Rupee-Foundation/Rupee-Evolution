...
{
    return GetBoolArg("-testnet", false) ? 64635 : 64634;
}
...
{
    //Ring Sigs - R u p e e E v o l u t i o n
...
Value stop(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Stop RupeeEvolution server (and possibly override the detachdb config value).");
    // Shutdown will take long enough that the response should get back
    if (params.size() > 0)
        bitdb.SetDetach(params[0].get_bool());
    StartShutdown();
    return "RupeeEvolution server stopping, please wait a few minutes for full shutdown...";
}
...
{ //  name                      function                 safemd  unlocked
  //  ------------------------  -----------------------  ------  --------
    ...
    // Ring Signatures -R u p e e E v o l u t i o n- v3.1.0
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: rupeeevolution-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
static string HTTPReply(int nStatus, const string& strMsg, bool keepalive)
{
    if (nStatus == HTTP_UNAUTHORIZED)
        return strprintf("HTTP/1.0 401 Authorization Required\r\n"
            "Date: %s\r\n"
            "Server: rupeeevolution-json-rpc/%s\r\n"
...
void ThreadRPCServer(void* parg)
{
    ...
    RenameThread("rupeeevolution-rpclist");
...
void ThreadRPCServer2(void* parg)
{
        ...
        (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use rupeeevolutiond";
...
void ThreadRPCServer3(void* parg)
{
    ...
    RenameThread("rupeeevolution-rpchand");
