...
int nCoinbaseMaturity = 5; //15 on Mainnet R u p e e E v o l u t i o n
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    */
    ...
    // Once upon a time, People were really interested in INN.
    ...
    // People wanted fortunastakes... Really Bad. But King Carsen was already busy with the rest of INN
    ...
    // many of thy were so invested in their stakes they pushed it, to get all the RUPEE they could. the streets
    // were dark and the days were long. people wanted a fair hand. they wanted to know they could rely
    // on the RUPEE to bring them joy and happiness, and not worry for when they might next taste the INN

    // and oh ye of little faith, feast your eyes upon the broth of thine calling. the hybrid stakes are no more.
    // gone are the days of not knowing when to expect the sweet caress of the glorious RUPEE to be gracing the silver linings
    // of your wallet. forever more you shall know the INN, and the RUPEE shall know you, and ye shall be fairly judged
    // for all of eternity
                            ...
                            if (pubScript == payee) {
                                ...
                                BOOST_FOREACH(CFortunaStake& mn, vecFortunastakes)
                                {
                                    ...
                                    if (vtx[1].vout[i].scriptPubKey == pubScript)
                                    {
                                        ...
                                        if (fDebug) printf("CheckBlock-POS() : Fortunastake PoS payee found at block %d: %s who got paid %s RUPEE rate:%"PRId64" rank:%d lastpaid:%d\n", pindex->nHeight, address2.ToString().c_str(), FormatMoney(value).c_str(), mn.payRate, mn.nRank, mn.nBlockLastPaid);
                ...
                if (pindex != NULL)
                {
                    ...
                    for (unsigned int i = 0; i < vtx[0].vout.size(); i++) {
                        ...
                        if(vtx[0].vout[i].nValue == fortunastakePaymentAmount )
                        {
                            ...
                            CBitcoinAddress mnAddress(mnDest);
                            if (fDebug) printf("CheckBlock-POW() : Found fortunastake payment: %s RUPEE to %s.\n",FormatMoney(vtx[0].vout[i].nValue).c_str(), mnAddress.ToString().c_str());
                            ...
                            BOOST_FOREACH(CFortunaStake& mn, vecFortunastakes)
                            {
                                ...
                                if (payee == pubScript)
                                {
                                    ...
                                    printf("CheckBlock-POW() : Found fortunastake payment: %s RUPEE to burn address.\n", FormatMoney(vtx[1].vout[i].nValue).c_str());
                                    foundPayee = true;
                                }
                            }
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xc66a5a16dac8ecdb245d2c8d51102b4e1c1480986cc3093377a26637175a635d"));///
        block.print();
...
unsigned char pchMessageStart[4] = { 0x7b, 0x32, 0xaf, 0xc3 };
