...
void Shutdown(void* parg)
{
    ...
    RenameThread("rupeeevolution-shutoff");
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("RupeeEvolution version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  rupeeevolutiond [options]                     " + "\n" +
                  "  rupeeevolutiond [options] <command> [params]  " + _("Send command to -server or rupeeevolutiond") + "\n" +
                  "  rupeeevolutiond [options] help                " + _("List commands") + "\n" +
                  "  rupeeevolutiond [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "rupeeevolution:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("RupeeEvolution"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("RupeeEvolution"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: rupeeevolution.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: rupeeevolutiond.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 64630 or testnet: 64631)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. RupeeEvolution is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  RupeeEvolution is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("RupeeEvolution version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "RupeeEvolution server starting\n");

    int64_t nStart;

    // Anonymous Ring Signatures ~R u p e e E v o l u t i o n- v3.0.0.0
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("RupeeEvolution"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        };
...
#ifdef USE_NATIVETOR
    // Native Tor Integration Continued -R u p e e E v o l u t i o nv3
    if(fNativeTor)
    {
        CService addrBind;
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        if (nLoadWalletRet == DB_TOO_NEW)
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of RupeeEvolution") << "\n";
        else
        if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart RupeeEvolution to complete") << "\n";
    ...
    if (!CheckDiskSpace())
    {
        return InitError(_("Error: not enough disk space to start RupeeEvolution."));
    }
